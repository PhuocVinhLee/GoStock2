...
//     const value = Math.floor(Math.random() * (100 - 50) + 50); // Random value between 50 and 100
//     if( (value)  ){
//       data.push({
...
const generateData = (): DataPoint[] => {
  ...
  for (let i = 0; i < 7; i++) { // Loop for 7 days
    for (let j = 0; j < 7; j++) { // 7 values per day
      const date = new Date(startDate.getTime() + (i * 24 * 60 * 60 * 1000) + (j * 3 * 60 * 60 * 1000));
      const dayOfWeek = daysOfWeek[date.getDay()];
      const value = Math.floor(Math.random() * (100 - 50) + 30); // Random value between 30 and 100
      data.push({
        date: date.toISOString(),
        dayOfWeek,
        value,
...
const generateData2 = (): DataPoint[] => {
  const data: DataPoint[] = [];
  const daysOfWeek = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
  const startDate = new Date(2023, 7, 23); // August 23, 2023 (Wednesday)

  for (let i = 0; i < 7; i++) { // 7 days
    for (let j = 0; j < 24; j++) { // 24 hours in a day
      const date = new Date(startDate.getTime() + i * 24 * 60 * 60 * 1000 + j * 60 * 60 * 1000);
      const dayOfWeek = daysOfWeek[date.getDay()];
      const value = Math.floor(Math.random() * (100 - 50) + 30); // Random value between 30 and 100
      data.push({
        date: date.toISOString(),
...
const CustomXAxisTick: React.FC = ({ x, y, payload }: any) => {
